...
> 事前に §7 の本番チェックリスト適用（`APP_ENV=production` でデモユーザーは投入されなくなる）
> **に加えて**、Cloudflare Access（Zero Trust）や IP 制限等のエッジ側アクセス制御の併用を必須とする。
> 「一時的だから」と local のまま公開を続ける運用を常態化させない。
