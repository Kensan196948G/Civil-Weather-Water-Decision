...
## 0. セッション開始時の自動実行（CTO 全権委任が要求された場合のみ）

ユーザーが「自律開発を進めて」「CTO 全権委任で」「/goal で進めて」など、
自律的な継続開発を明示的に依頼した場合に限り、以下のステップを実行してください。
単なる質問・状態確認・小規模な修正依頼など、自律モードを前提としないやり取りでは、
このセクションを発動せず通常の対話として応答してください（読み取り専用の状態確認
— `state.json` の読み取りや `gh issue/pr list` 等 — は自律モード開始判断に関わらず
必要な範囲で行って構いません）。

自律モード開始が確認できた場合のみ、次のステップへ進みます。
...
`/codex:setup` は外部認証を伴うコマンドのため、自律モード開始が確認できている
場合にのみ実行すること。
...
- CTO が「デプロイ準備完了」と判断したら `project.deploy_plan`（`state.json` 契約に実在するキー）に
  状態・手順を記録し、手順書を自動生成する
- 実際のデプロイは**人間（ユーザー）が手動**で実行する（CTO はデプロイを自動実行しない）
- デプロイ完了後: `project.current_phase` を保守フェーズへ更新 → **無期限保守フェーズへ移行**
...
| 状態 | 処理 |
|---|---|
| STABLE 達成 | PR 仕上げ → 人間へ「マージしますか？ [y/N]」確認 → (承認後) merge → デプロイ手順書提示（実デプロイは人間が実行）→ 終了報告 |
