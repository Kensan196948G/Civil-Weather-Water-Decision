...
- [50年確率波 極値解析（デモ）設計](./docs/design/extreme-value-wave50.md)
...
- [MVP/Prototype 評価・改善報告書（2026-08-13）](./docs/evaluation/2026-08-13-mvp-assessment.md)
...
⚙️ 管理           閾値管理 / データ取得状況 / 運用状態 / レポート出力 / 監査ログ / ユーザー管理 / 設定（通知・保存期間・AI設定）
```
※ 50年確率波・河川観測の自動取得はデモ・シミュレーションデータで稼働中（設計用途には
使用不可。NOWPHAS / 水防災オープンデータ提供サービスの接続はエピック #72 段5-7）。
管理系はロール制御（閾値管理・監査ログ・運用状態=admin/技術管理者、ユーザー管理・設定=admin）。

### 判断履歴の検索・類似判断・PDF帳票（2026-08-13）

- 判断履歴画面に検索（現場名・メモ・記録者・工種）・現場/工種絞り込みツールバーを追加
- 類似する過去判断の参照（同一現場・工種・判定レベル・直近7日をスコア順に表示）
- 判断履歴の PDF 帳票出力（A4横・日本語。CSV と同様に認証・現場権限で出力。
  フォントは SIL OFL 1.1 の M+ 1p を `backend/app/assets/fonts/` に同梱・self-host）
- ユーザー管理画面（管理者専用）: 一覧 / 作成 / ロール変更 / 有効・無効化 / パスワードリセット / 削除。
  自分自身の降格・無効化・削除と最後の有効な管理者の変更を防止し、変更は監査ログへ記録
...
| テスト状況 | backend 481 / frontend（logic 21 + adapter契約 59 + policy系 3 + asset/policy suites）、全pass（2026-08-13時点） |
...
- ✅ デモ自動取得（DEMO-RIVER・シミュレーション）の10分間隔ジョブと
  `data_source_statuses`（DS-RIVER-DEMO）への反映（UI/APIで「デモ」であることを明示）
- ✅ 河川観測画面に全国マップ・観測所最新値・デモ自動取得ステータス表示
- ✅ 判定エンジンへの実測値組み込み（#112: 水位トレンド・上流雨量・STALE→レベル3。手動実測ベース）
- ❌ 公式自動取得（水防災オープンデータ提供サービス等）は未接続（デモ値は判定には
  反映するが、退避判断は公式サイトと現地確認を優先）
