...
- 左側サイドメニューで、ダッシュボード以外の項目（現場管理 / 気象・海象データ /
  施工判定 / 分析 / 管理）をアコーディオン式（グループ見出しクリックで開閉）に変更。
- 開くグループは1つ（他グループを開くと前のグループは閉じる）。
- 画面遷移時はアクティブ項目を含むグループを自動で開く。手動の開閉状態は
  遷移以外で上書きしない。
- ログアウト時に開閉状態をリセット（前ユーザーの状態を残さない）。

### 検証結果

| 項目 | 結果 |
| --- | --- |
| e2e smoke | PASS（ログイン→各グループ経由で運用状態/海象/現場一覧/WBGT/海上作業） |
| 地図回帰 | map_repro PASS（ダッシュ16・現場詳細5+path・判断・海象16） |
| frontend tests | adapter契約 51 passed / logic 21 / policy系 6 / node --check PASS |
| CI（PR #137） | 全5ジョブ success |
| 本番反映 | 2026-08-09 16:48 JST、main反映（71a3743）＋ frontend 再起動 |
| 本番スモーク | frontend 200 / public 302 / data-adapter.js にアコーディオン実装反映を確認 |
